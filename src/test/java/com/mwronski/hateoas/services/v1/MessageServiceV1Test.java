...
/**
 * Test cases for message service in version 1.x
 *
 * @author Michal Wronski
 * @version 1.0
 * @date 31-05-2014
 * @see com.mwronski.hateoas.services.v1.MessageServiceV1
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(classes = {RestApplicationTest.class})
@WebAppConfiguration
public class MessageServiceV1Test extends ReadServiceTest<Message> {
    ...
    public void shouldFindElementWithProperFieldsReturned() throws Exception {
        ...
        result.andExpect(jsonPath("$.entityId").value("123-v1"));
        result.andExpect(jsonPath("$.title").value("Title-v1"));
    ...
    public void shouldGetListOfElementsWithProperFieldsReturned() throws Exception {
        ...
        result.andExpect(content().contentType(acceptVndVersion()));
        //and elements have proper fields set
        result.andExpect(jsonPath("$.resources[0].entityId").value("123-v1"));
        result.andExpect(jsonPath("$.resources[0].title").value("Title-v1"));
        result.andExpect(jsonPath("$.resources[0].content").doesNotExist());
    ...
    protected String acceptVndVersion() {
        return "application/vnd.messages-v1+json";
    }

    @Override
    protected Message sampleElement() {
        Message sample = new Message();
        sample.setEntityId("123-v1");
        sample.setTitle("Title-v1");
        sample.setContent(null); //not supported field
        sample.setSender(null);  //not supported field
        return sample;
    }
}
